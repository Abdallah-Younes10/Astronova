...
var uni = document.getElementsByClassName('universe');
...
window.onscroll = function(){
    if(window.scrollY > 500){
        for (var i = 0; i < uni.length; i++) {
            uni[i].classList.remove('universehid');

        }
    }
    else{
        for (var i = 0; i < uni.length; i++) {
            uni[i].classList.add('universehid');
        }
    }
	...
	if(window.scrollY > 4600){
        for (var i = 0; i < unit.length; i++) {
			...
			image.src = 'Images/the-suit-new-EVA.png';
        }
    }
    else{
        for (var i = 0; i < unit.length; i++) {
            
			image.src = 'Images/the-suit-new.png';
...
window.onload = function() {
    // Check if the video was already played in this session
    const videoPlayed = sessionStorage.getItem('videoPlayed');
    
    if (videoPlayed) {
      // If the video was played, skip the preloader and show main content
      document.getElementById('preloader').style.display = 'none';
      document.getElementById('main-content').style.display = 'block';
    } else {
      // If the video was not played, play it and then set it as played
      const introVideo = document.getElementById('introVideo');
      introVideo.addEventListener('ended', function () {
        // Hide video and preloader
        document.getElementById('preloader').style.display = 'none';
        document.getElementById('main-content').style.display = 'block';
        // Set that video was played
        sessionStorage.setItem('videoPlayed', true); // Store the info that the video was played
      });
    }
  }
